...
class SAC(Agent):
    ...
    params: nn.FrozenDict = struct.field(pytree_node=False)
    ...
    def create(
        ...
        actor_head = GaussianHead(action_shape=hparams.action_space.shape)

        buffer = ReplayBuffer(hparams.replay_memory_size)
        params_target: nn.FrozenDict = self.params.copy({})
        dim_actions = hparams.action_space.shape[0]
