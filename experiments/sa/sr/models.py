...
SrOutput = collections.namedtuple(
    "SrOutput", ["output", "synthetic_return", "augmented_return", "sr_loss"]
)
EmState = Tuple[Array, Array]
SrState = Tuple[EmState, object]
...
class EpisodicMemory(hk.RNNCore):
    ...
    def __call__(self, inputs: Array, prev_state: EmState):
    ...
    def initial_state(self, batch_size: int) -> EmState:
...
class SyntheticReturnsCoreWrapper(hk.RNNCore):
    ...
    def __init__(
        ...
        name="synthetic_returns_wrapper",
    ):
        """Constructor.
    ...
    def __call__(
        self, inputs: Tuple[Array, Array], prev_state: SrState
    ) -> Tuple[SrOutput, SrState]:
